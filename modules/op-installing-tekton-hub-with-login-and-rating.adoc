...
<11> GitHub Enterprise URL, if you are authenticating using GitHub Enterprise. Do not give the URL to the catalog as a value for this field.
<12> GitLab Enterprise URL, if you are authenticating using GitLab Enterprise. Do not give the URL to the catalog as a value for this field.
+
[NOTE]
====
...
----
<1> The namespace in which you install {tekton-hub}; default is `openshift-pipelines`.
...
<6> Optional: Additional users. You can mention many users, such as `[<username_1>, <username_2>, <username_3>]`.
