...
----
+
This query returns the total GB ingested per day, measured at the Loki distributor (the entry point for all logs). Using a 24-hour window provides a more accurate daily average than shorter sampling periods.

. Compare the result against the sizing table and choose the appropriate size:
+
[source,terminal]
----
# Example: Query returns 421 GB
