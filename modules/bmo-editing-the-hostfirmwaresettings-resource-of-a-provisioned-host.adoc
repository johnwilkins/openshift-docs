...
You can edit the `HostFirmwareSettings` for a provisioned host, delete the host from the machine set, scale down the machine set, and then scale up the machine set to make the changes take effect.
...
. Edit a host's `HostFirmwareSettings` resource by running the following command:
+
[source,terminal]
----
