...
[role="_abstract"]
You can adjust LokiStack capacity by changing the overall t-shirt size or by scaling individual component replicas. Understanding when to use each approach helps you optimize resources and maintain performance.
...
Changing the size triggers a rolling restart of all Loki components. Plan sizing changes during maintenance windows.
...
[IMPORTANT]
====
Component replica overrides persist across size changes. If you change from `1x.small` to `1x.medium`, the `querier` and `queryFrontend` overrides remain at 4 and 3 replicas instead of adopting the `1x.medium` defaults. Remove explicit replica settings to return to size-based defaults.
====
...
|Cluster grows significantly (node count doubles)
|Change t-shirt size
...
* xref:../installing/loki-deployment-sizing.adoc#calculating-sizing-after-installation_understanding-loki-sizing[Calculating sizing after installation]
