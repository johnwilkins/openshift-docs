...
`token`:: The authentication token that the `oc create token` command created. If you provide this token, it overrides the `service_account` setting and `opc` uses this token to authenticate.
...
`service_account`:: The name of a service account for authenticating with the {tekton-results} API. If you provided the authentication token, you do not need to provide the `service_account` parameters.
+
