...
* With this update, you can reference parameters and results by using a bracket notation with single or double quotes. before this update, you could only use the dot notation. For example, the following are now equivalent:
...
* With this update, the `feature-flag-triggers` config map has a new field `labels-exclusion-pattern`. You can set the value of this field to a regular expression (regular expression) pattern. The controller filters out labels that match the regular expression pattern from propagating from the event listener to the resources created for the event listener.
...
* With this update, you can find and use {pac} (PAC) as an add-on option.
+
[IMPORTANT]
====
{pac} is a Technology Preview feature.
====
...
With {pac}, cluster administrators and users with the required privileges can define pipeline templates as part of source code Git repositories. When triggered by a source code push or a pull request for the configured Git repository, the feature runs the pipeline and reports status.
...
* When you run Maven and Jib-Maven cluster tasks, the default container image is supported only on Intel (x86) architecture. Therefore, tasks will fail on ARM, {ibmpowerProductName} Systems (ppc64le), {ibmzProductName}, and LinuxONE (s390x) clusters. As a workaround, you can specify a custom image by setting the `MAVEN_IMAGE` parameter value to `maven:3.6.3-adoptopenjdk-11`.
// issue # is unknown.
+
[TIP]
====
...
* On {ibmpowerProductName} Systems, {ibmzProductName}, and LinuxONE, the `s2i-dotnet` cluster task is unsupported.
// issue # is unknown.
...
* With this update, the race condition in case listener does not happen when multiple interceptors return extensions.
