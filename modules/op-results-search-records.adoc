...
[role="_abstract"]
You can search for records by using Common Expression Language (CEL) queries. As each record contains full YAML information for a pipeline run or task run, you can find records by many different criteria.
...
----
+
Replace `<namespace_name>` with the namespace in which you created the pipeline runs or task runs. Alternatively, search for records within a single result by entering the following command:
+
[source,terminal]
----
