...
By default, {pac} scopes the access token only to the repository from which {pac} retrieves the pipeline definition. In some cases, you might want the token to have access to additional repositories. For example, there might be a CI repository where the `.tekton/pr.yaml` file and source payload reside, but the build process defined in `pr.yaml` fetches tasks from a separate private CD repository.
...
----
+
In this example, the `Repository` custom resource applies to the `linda/project` repository in the `test-repo` namespace. {pac} extends the scope of the generated GitHub token to the `owner/project` and `owner1/project1` repositories, and the `linda/project` repository. These repositories must exist under the `test-repo` namespace.
+
[NOTE]
====
...
If you provide both global configuration and repository level configuration, {pac} scopes the token to all the repositories from both configurations, as in the following example.
