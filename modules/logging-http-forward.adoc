...
`format`:: Optional: Data format for sending logs. Valid values are `json` (default) and `ndjson` (newline-delimited JSON). Use `ndjson` when forwarding to systems that expect newline-delimited JSON format, such as VictoriaLogs or other log aggregation systems that process logs line by line.
