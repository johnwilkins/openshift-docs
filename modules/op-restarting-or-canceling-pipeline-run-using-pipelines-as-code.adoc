...
[NOTE]
====
If you run a command on a commit that exists in multiple branches within a push request, {pac} uses the branch with the latest commit.
...
* If you include a branch specification, such as `/test branch:user-branch`, {pac} performs the test on the commit that contains the comment with the context of the `user-branch` branch.
====
