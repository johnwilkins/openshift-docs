...
If you start a task run directly, without the use of a pipeline run, {tekton-results} creates a result for this task run. This result contains the record for the same task run.
...
Each record has a name that includes name of the result that contains the record, and the UUID of the `PipelineRun` or `TaskRun` CR to which the record corresponds. The format for the result name is `<namespace_name>/results/<parent_run_uuid>/results/<run_uuid>`.
...
The record includes the full YAML manifest of the `TaskRun` or `PipelineRun` CR as it existed after the completion of the run. This manifest contains the specification of the run, any annotation that you specify for the run, and certain information about the results of the run, such as the time when the run completed and whether it was successful.
