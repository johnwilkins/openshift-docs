...
* *Storage backend:* Cloud-managed object storage is available (S3, GCS, Azure Blob Storage)
...
* *Sizing:* Choose initial size based on expected log volume. See xref:../installing/loki-deployment-sizing.adoc#loki-deployment-sizing[Loki deployment sizing] for guidance based on GB/day ingestion rates.

*On-premise deployment (bare metal, VMware)*::
* *Storage backend:* Requires S3-compatible object storage. Common options include NetApp StorageGRID, Ceph RGW, or other enterprise S3-compatible storage solutions.
...
* *Log storage:* Forward logs to a central cluster's `LokiStack`, or deploy local `LokiStack` with S3-compatible storage and limited retention
...
* *Sizing:* Choose size based on expected log volume. See xref:../installing/loki-deployment-sizing.adoc#loki-deployment-sizing[Loki deployment sizing] for guidance based on GB/day ingestion rates. Consider enabling ruler for alerting and retention policies.
