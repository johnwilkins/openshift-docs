...
* With this update, you can use an data enumeration declaration to limit the values that you can supply for a parameter of a pipeline or task, as shown in the following example. To use this feature, you must set the `pipeline.options.configMaps.feature-flags.data.enable-param-data enumeration` spec in the `TektonConfig` CR to `true`.
+
.Example usage
[source,yaml]
----
...
kind: Pipeline
metadata:
  name: pipeline-param-data enumeration
spec:
  params:
  - name: message
    data enumeration: ["v1", "v2"]
    default: "v1"
# ...
----
...
* Before this update, when a pipeline run was canceled, {tekton-results} did not record the logs of this pipeline run. With this update, {tekton-results} records the logs of a canceled pipeline run.
