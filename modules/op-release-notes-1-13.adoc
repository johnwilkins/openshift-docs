...
* To enable keeping pods when a task run is canceled, along with setting the `pipeline.options.configMaps.feature-flags.data.keep-pod-on-cancel` spec to `true` in the `TektonConfig` CR, you also need to set the `pipeline.enable-api-fields` spec to `alpha` in the `TektonConfig` CR.

* If you enable keeping pods when a task run is canceled, when a task run is canceled because of a default timeout or because you set a timeput in the pipeline specification, {pipelines-shortname} deletes the pod.
